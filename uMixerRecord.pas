...
type
  ...
  private
    fMsgHandlerHWND: HWND;     //创建的WINDOW用于接收消息
    hWaveIn: HWaveIn;
...
constructor TMixerRecord.Create;
begin
  fMsgHandlerHWND := 0;
  fMsgHandlerHWND := AllocateHWnd(WMWIMReceive);
end;

destructor TMixerRecord.Destroy;
begin
  if fMsgHandlerHWND <> 0 then
  begin
...
{指定要录制的格式}
  ...
  begin
    ...
    Sleep(500); //网上搜索的我找到的代码是调用 waveInReset 之后 Sleep(500) 再调用 waveInClose
  ...
  nRet := waveInOpen(@hWaveIn, WAVE_MAPPER, @fmt, fMsgHandlerHWND, 0, CALLBACK_WINDOW);
  if nRet = 0 then
  begin
